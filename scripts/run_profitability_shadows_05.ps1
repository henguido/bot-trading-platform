...
Write-Host "[SHADOW-05] Ejecuta 05A + diagnosticos 05A/05F + scanner 05I + challenger 05J + ejecucion prospectiva 05K."
Write-Host "[SHADOW-05] 05C/05D/05G/05H quedan como diagnostico historico y no capturan nuevas cohortes."
Write-Host "[SHADOW-05] 05K no hace backfill: solo entra si acaba de capturarse un bucket 05I fresco."
...
while ($true) {
    ...
    python scripts\shadow_strategy_execution_05k.py
    if ($LASTEXITCODE -ne 0) { throw "05K fallo con codigo $LASTEXITCODE" }
